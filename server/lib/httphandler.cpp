...
void HttpHandler::HandleBunnyHTTPRequest()
{
	...
	if (uri.startsWith("/ojn_api/"))
	{
		...
		if(httpApi)
		{
			std::unique_prt<ApiManager::ApiAnswer> apianswer(ApiManager::Instance().ProcessApiCall(uri.mid(9), request));
	...
	else if (uri.startsWith("/ojn/FR/api"))
	{
		...
		if(httpVioletApi)
		{
			std::unique_prt<ApiManager::ApiAnswer> apianswer(ApiManager::Instance().ProcessApiCall(uri, request));
			QByteArray answer = apianswer->GetData();
			incomingHttpSocket->write(answer);
